...
# Filter Exceptions
class FilterError(ProductError):
    """Base class for all filter-related exceptions."""
    def __init__(self, message=None, code=None):
        self.message = message or "An unspecified filter error occurred."
        self.code = code or "FILTER_ERROR"
...
class InvalidFilterError(FilterError):
    ...
    def __init__(self, errors=None, message=None):
        self.errors = errors
        self.message = message or f"Invalid filter parameters: {errors}"
        self.code = "INVALID_FILTER"
        super().__init__(self.message, self.code)

class InvalidOrderingError(FilterError):
    """Raised when ordering parameters are invalid."""
    def __init__(self, field=None, message=None):
        self.field = field
        self.message = message or f"Invalid ordering field: {field}"
        self.code = "INVALID_ORDERING"
        super().__init__(self.message, self.code)

class InvalidSearchError(FilterError):
    """Raised when search parameters are invalid."""
    def __init__(self, term=None, message=None):
        self.term = term
        self.message = message or f"Invalid search term: {term}"
        self.code = "INVALID_SEARCH"
        super().__init__(self.message, self.code)
...
class _Exceptions:
    ...
    InvalidOrderingError = InvalidOrderingError
